...
def preprocess(rawdata, donormalize=0, istrain=1, traininfo=None, doengr=0):
    ...
    print('raw data size (rows,columns)', dataframe.shape)
    ...
    if len(intersect([outfeature],allfeats)) == 1: 
        allfeats = allfeats[:-1] #all features but the last row
    ...
    if doengr == 1:
        ...
        # Fix outliers --> Feature engineering
        # IQR --> Inter-quartile Range
        # Replace with quantile Values : IQR proximity rule.
        # IQR proximity rule states that a value is an outlier if it falls outside the lower and upper boundary
        # Upper boundary = 75th quantile + (IQR * 1.5), Lower boundary = 25th quantile - (IQR * 1.5)
        # IQR = 75th quantile - 25th quantile
        # dataframe.columns
        for feats in normfeats:
            q1 = dfcpy[feats].quantile(0.25) # 25th quantile
            q3 = dfcpy[feats].quantile(0.75) # 75th quantile
            iqr = q3-q1
            extlow = q1 - (1.5*iqr) # Lower Boundary
            exthigh = q3 + (1.5*iqr) # Upper Boundary
    ...
    # for the select features: make a copy of the names
